...
export const vendorCommercialData: Record<string, VendorCommercial> = {
  ...
  fullenrich: {
    ...
    capability: "enrich-only",
    commercialNextStep: "CONTRACT SIGNED Aug 31 — Austin confirmed in #pod-enrichment ('fullenrich contract should be signed today' at 9:52am, 'this has been signed!!' by ~4:14pm; Ross: 'amazing, thanks!'). No further legal action needed. Next: loop in eng — Ross Alexander flagged FullEnrich contract signature as the enrichment pod's only hard blocker (per Aug 30 #epd-weekly-updates), so integration/implementation (schema, provider registration, host function wiring already parallelized) should now unblock; targeting GA per pod's prior 'build and ship pending contract' plan. Prior history: Aug 6 Byron sent DPA response — deal '95% there' with 2 items (2x liability cap, direct DSR handling); Aug 7 DSR ask confirmed acceptable; Aug 19-20 liability supercap negotiated to $250K/3x with DPA-precedence resolved; Aug 26 Byron called it 'done-done'; Aug 27 Austin confirmed signing imminent.",
    commercialOwner: "haley",
    annualBudgetUsd: null,
    budgetStatus: "signed",
    estimatedAnnualVolume: null,
