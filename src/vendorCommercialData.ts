...
export const vendorCommercialData: Record<string, VendorCommercial> = {
  ...
  sixsense: {
    ...
    capability: "enrich-only",
    commercialNextStep: "Aug 26: Micki Howl sent a Zoom meeting invite directly ('6sense x Unify — formalizing our API relationship', CC: jq@6sense.com, eschori@insightpartners.com) rather than waiting on Gwen's scheduling outreach — confirm exact date/time from the calendar invite and attend. Bring usage/growth trajectory; get 6sense's proposed volume-tier structure and negotiate. Also reconcile against the Aug 20 CreatorIQ handoff note claiming '6sense signed ~mid-2026' — conflicts with 6sense's own 'no current signed order form' claim.",
    commercialOwner: "haley",
    annualBudgetUsd: null,
  ...
  retentiondotcom: {
    ...
    capability: "enrich-only",
    commercialNextStep: "Skip for now — deprioritized for the website-visitor-ID SKU tracked here. Aug 11 meeting ('B2B contact data and validation strategy with founder') surfaced a separate personal-email/validation offering not covered by this pricing: Retention.com's differentiator is 9 years of consumer-to-business personal-email identity linkage, and Unify's stated interest order was (1) incremental work email/mobile coverage, (2) personal email — revisit in a few months, (3) web deanonymization. No commercial motion on that yet; revisit scope/status before next contact.",
    commercialOwner: null,
    annualBudgetUsd: null,
    budgetStatus: "excluded",
    estimatedAnnualVolume: null,
  ...
  fullenrich: {
    ...
    capability: "enrich-only",
    commercialNextStep: "CONTRACT SIGNED Aug 31 — Austin confirmed in #pod-enrichment ('fullenrich contract should be signed today' at 9:52am, 'this has been signed!!' by ~4:14pm; Ross: 'amazing, thanks!'). No further legal action needed. Sep 1 (#partnership-unify-fullenrich): Haley asked Charline to set up Ross with another workspace and provision credits there. Sep 2 (#pod-enrichment, Ross Alexander weekly update): integration PRs have begun; work could slip to next week depending on pace, Ross trying to ship this week. Sep 2 (#partnership-unify-fullenrich): Byron added 2,000 test credits to Ross's workspace. Sep 2, 8:21pm (#pod-enrichment, Austin): proposed final waterfall placement — FullEnrich 2nd-to-last for emails, 2nd (behind Forager) for phones; also asked Haley to price FullEnrich per Unify's standard margin if needed. Sep 3 (#pod-enrichment, 'New vendor contracts' thread): ADL flagged the signed contract PDF is missing from Ramp's contract-management system, which is delaying finance's invoice approval — Austin holds the signed copies; Haley to retrieve and forward to contracts@ramp.com. Sep 4, 1:38pm (same #pod-enrichment thread): Haley confirmed the FullEnrich contract is now correctly showing in Ramp's contracts tab; ADL acknowledged but asked Gwen to confirm it's set up correctly on Tuesday — resolved pending that check. Sep 8 (#epd-weekly-updates, Ross Alexander): integration now 'in progress' — bulk enrichment implementation + internal rollout targeted by Wed, GA by end of week — but flagged under Risks/Blockers that 'FullEnrich GA is dependent on receiving contracted API key - in contact with the team, but still awaiting.' Despite the signed contract, engineering does not yet have the production API key needed to ship; chase FullEnrich (Byron) directly for the key to unblock GA. Prior history: Aug 6 Byron sent DPA response — deal '95% there' with 2 items (2x liability cap, direct DSR handling); Aug 7 DSR ask confirmed acceptable; Aug 19-20 liability supercap negotiated to $250K/3x with DPA-precedence resolved; Aug 26 Byron called it 'done-done'; Aug 27 Austin confirmed signing imminent.",
    commercialOwner: "haley",
    annualBudgetUsd: null,
    budgetStatus: "signed",
    estimatedAnnualVolume: null,
    coverageNote: "Work email + mobile + personal email enrichment — key waterfall supplement vendor",
    financeNote: "Both tiers were six-figure commitments ($250K-$400K/yr combined real-time + flat-file); final tier signed not yet confirmed in these sources. Contract fully executed Aug 31. Sep 8: the signed contract didn't unblock the production API key — a separate unpaid Sep 1 invoice (#QMI1UJD4-0007) was the actual gate; Unify processed payment via Ramp Sep 8, still pending final approval/posting on FullEnrich's side.",
    legalStatus: "contract-signed",
    resellAgreementStatus: "ready-to-sign",
  ...
  unipile: {
    ...
    capability: "platform",
    commercialNextStep: "Aug 4 3:38pm (#product-linkedin-automation): Haley now has Unipile's legal agreement in hand (alongside HeyReach's) but hasn't sent either out for review yet — she asked Kevin Liang/James Grinage/Joe Lipshutz when the team wants to converge on a vendor choice between HeyReach and Unipile, and offered to scope the legal review this week or next once that lands. Aug 5: Kevin Liang + Connor Heggie aligned internally to move forward with Unipile for the native build and Kevin started a POC — this unblocks the 'when to converge' question directionally, but Haley has not yet sent Unipile's agreement out for legal review. Next: confirm with Kevin/Connor/James whether this POC-stage lean should now trigger sending Unipile's legal agreement to Crosby, and let HeyReach know the direction once the team is sure.",
    commercialOwner: "haley",
    annualBudgetUsd: null,
    budgetStatus: "exploring",
    estimatedAnnualVolume: null,
