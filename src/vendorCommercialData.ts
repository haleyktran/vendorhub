...
export const vendorCommercialData: Record<string, VendorCommercial> = {
  ...
  twilio: {
    ...
    pricingTldr: "Two-leg call cost $0.018/min ($0.004 SDK + $0.014 PSTN); AMD $0.0075/call; recording $0.0025/min; transcription $0.035/min; phone numbers $1.15/mo (volume pricing after 1,000). $150K annual commit unlocks discounted rates — internal consensus (Jul 30) is to go committed and negotiate the number down.",
    pricingDetail: "Committed rate is a floor, not a cap — overage bills at the same discounted rate. Quarterly commitments available (e.g. $37.5K/quarter); ramped commitments possible for startups. Two negotiation levers: lower commit = worse rate but less risk, higher commit = better rate but requires confidence in dialer scale. Dialer early signals are positive, leaning toward committing. Not a vendor Unify would switch off — negotiation is purely about price and timing.",
    capability: "platform",
    commercialNextStep: "Jul 31 (Slack DM, James/Haley): James deprioritized this — told Haley to disregard his Twilio contract ask after syncing with Austin, citing higher-leverage work elsewhere (Coresignal, FullEnrich) as the priority. Aug 1: Haley told James she's still happy to hop on a call with Twilio if useful, so this isn't fully shelved — just not actively being pushed by Haley this week. No formal contract review started yet, only email exchange as of Jul 30. Whenever resumed: reply to Twilio thread requesting the order form (with upfront commit options), MSA, and DPA; project dial volume for the next year to set a conservative commit number; loop in Crosby for legal review once docs arrive.",
    commercialOwner: "haley",
    annualBudgetUsd: 150000,
    budgetStatus: "exploring",
    estimatedAnnualVolume: null,
    ...
    financeNote: "$150K/yr minimum commit under negotiation; James Grinage wants both the commit level and per-minute/per-number rates pushed lower than Twilio's proposal. Haley looped in by James (Jul 30) to help drive the negotiation alongside Ami. Jul 31: James deprioritized his own ask given higher-leverage vendor work (Coresignal/FullEnrich); Haley remains open to a Twilio call if it comes up.",
    legalStatus: "not-started",
    resellAgreementStatus: null,
    legalNotes: "Jul 30 (Granola, Twilio Contract Negotiation): no formal contract yet, only email exchange — need to request order form, MSA, and DPA from Twilio. DPA required since Twilio processes personal data (phone numbers); unclear if one was ever previously signed. Jul 30 (Slack DM, James/Haley): James asked Haley to help negotiate and drive the process; Haley agreed, already pinged Ami, will research comparable rates before the call. Jul 31: James walked back the ask as lower priority than Coresignal/FullEnrich; Aug 1: Haley offered to still hop on a Twilio call if needed.",
  },
