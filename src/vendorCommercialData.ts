...
export const vendorCommercialData: Record<string, VendorCommercial> = {
  ...
  coresignal: {
    ...
    capability: "finds-enrich-dataset",
    commercialNextStep: "Pricing is resolved ($3,500/mo flat, 5% renewal cap). Aug 13: Coresignal's legal team rejected Unify's DPA (independent-controller argument) and sent back their MSA with an added clause codifying that position — Haley forwarded to Unify legal for review. Get ADL's sign-off (post-PTO) on accepting the no-DPA position based on 5x5/Salutary precedent, then repropose a liability supercap on the MSA to resolve the remaining sticking point — same approach being used with FullEnrich. Austin cc'd on comms while ADL is out. Separately, still need Paulius's corrected file + 28 zero-coverage profile URLs investigated before re-testing coverage, and outcome of Coresignal's internal Friday Data+Scraping strategy session on Posts limitations.",
    commercialOwner: "haley",
